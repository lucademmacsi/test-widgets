...
@Component({
  selector: 'yucca-input-datasetcolumn',
  templateUrl: './input-datasetcolumn.component.html',
  styleUrls: ['./input-datasetcolumn.component.sass']
})
export class InputDatasetcolumnComponent extends AbstractParamTypeComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.p.value.demo) { // If we want to edit
      this.column = JSON.parse(this.p.value.demo);
      this.suggestColumn = { "name": this.column.key }
    }
  }

  getValue() {
    if (!this.modalParams.hasCountingMode)
      delete this.column.countingMode;
    this.value = JSON.stringify(this.column);
  }
